  ...
  template: `
    <div class="bg-slate-900/60 border border-slate-800 rounded-xl p-5 backdrop-blur shadow-lg flex flex-col h-full relative overflow-hidden">
        ...
        @if (stadium) {
          <svg viewBox="0 0 800 500" class="w-full h-full max-h-[380px]" xmlns="http://www.w3.org/2000/svg">
            ...
            @if (stadium.mapType === 'oval-open-air') {
              ...
              <rect x="140" y="80" width="520" height="300" rx="60" fill="#0b1329" stroke="#334155" stroke-width="4" />
              <circle cx="400" cy="230" r="160" fill="none" stroke="#0ea5e9" stroke-width="2" stroke-dasharray="4 4" />
              <rect x="240" y="150" width="320" height="160" rx="16" fill="#020617" stroke="#1e293b" stroke-width="3" />
              ...
              <rect x="120" y="90" width="560" height="280" rx="10" fill="#0b1329" stroke="#334155" stroke-width="4" />
              <rect x="250" y="150" width="300" height="160" rx="6" fill="#020617" stroke="#1e293b" stroke-width="3" />
            }
            ...
            @for (gate of stadium.gates; track gate; let i = $index) {
              <g [attr.transform]="gatePosition(i, stadium.gates.length)">
                ...
                />
                <text y="5" font-size="10" font-weight="bold" [attr.fill]="isCriticalGate(i) ? '#fca5a5' : '#cbd5e1'" text-anchor="middle">
                  {{ gateShortLabel(gate) }}
                </text>
              </g>
            }
      ...
      @if (store.activeScenario() && store.latestResult(); as result) {
        <div class="mt-3 bg-slate-900/90 border border-slate-800 p-2.5 rounded-lg text-xs space-y-1.5 backdrop-blur-sm shrink-0">
