  ...
  template: `
    ...
    >
      <div class="flex justify-between items-center mb-4">
        <div>
          <h2 class="text-md font-bold uppercase tracking-wider text-slate-100">
            8-Agency AI Directive Board
          </h2>
        ...
        @if (store.severity(); as sev) {
          <span
            role="status"
            class="px-2.5 py-0.5 rounded border text-[10px] font-semibold uppercase tracking-wider bg-red-500/10 text-red-400 border-red-500/30 animate-pulse"
          >
            Severity: {{ sev }}
          </span>
        }
      ...
      @if (store.latestResult(); as result) {
          ...
          @for (agency of agencies; track agency.key; let i = $index) {
            ...
            >
              <div class="flex items-center space-x-2 mb-1.5">
                ...
                <span class="text-xs uppercase font-bold tracking-wider text-slate-300"
                  ><span aria-hidden="true">{{ agency.icon }}</span> {{ agency.label }}</span
                >
              </div>
        ...
        <div class="mt-3 bg-slate-950/40 border border-slate-800 p-3.5 rounded-lg">
          <div class="flex justify-between items-center mb-2">
            <span class="text-xs uppercase font-bold tracking-wider text-slate-300"
              ><span aria-hidden="true">📢</span> Multilingual Broadcast Scripts</span
            >
          </div>
          ...
          <p class="text-xs text-slate-300 italic leading-relaxed">
            "{{ result.multilingualScripts[activeLang()] }}"
          </p>
        </div>
        ...
        <div
          class="flex-1 flex items-center justify-center text-center py-8 text-xs text-slate-500 italic"
        >
