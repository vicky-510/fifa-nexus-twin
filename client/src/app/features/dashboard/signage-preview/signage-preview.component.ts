...
@Component({
  ...
  template: `
    <div class="bg-slate-900/60 border border-slate-800 rounded-xl p-5 backdrop-blur shadow-lg">
      <h2 class="text-xs font-bold uppercase tracking-wider text-slate-400 mb-3">Stadium Screen Preview</h2>

      @if (store.latestResult(); as result) {
        <div class="bg-black border-2 border-cyan-500/40 rounded-lg p-4 mb-3 shadow-[0_0_20px_rgba(6,182,212,0.15)]">
          <div class="text-red-400 text-xs font-bold mb-2">⚠️ {{ store.severity() || 'ALERT' }}</div>
          <div class="text-cyan-300 text-sm font-bold uppercase mb-1 leading-tight">{{ result.navigation }}</div>
          <div class="text-slate-400 text-[10px] uppercase tracking-wider mt-3">🚨 Operational Notice 🚨</div>
        </div>
        ...
        <div class="bg-black border-2 border-slate-800 rounded-lg p-4 text-center text-slate-600 text-xs italic">
          No active notice
        </div>
      }
    </div>
