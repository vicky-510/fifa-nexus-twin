...
@Component({
  ...
  template: `
    <div
      class="bg-slate-900/60 border border-slate-800 rounded-xl p-5 backdrop-blur shadow-lg flex flex-col justify-between h-full"
    >
      <div>
        <h2
          class="text-md font-bold uppercase tracking-wider text-amber-500 mb-4 flex items-center space-x-2"
        >
          <span
            class="w-2.5 h-2.5 rounded-full bg-amber-500 animate-ping"
            aria-hidden="true"
          ></span>
        ...
        <p class="text-xs text-slate-400 mb-5">
          Select a crisis vector relevant to this stadium's risk profile. GenAI will orchestrate
          8-agency directives.
        </p>

        <!-- Scenario Choices (data-driven from stadium.availableCrisisIds) -->
        <div
          class="space-y-2 mb-6 max-h-[260px] overflow-y-auto pr-1"
          role="radiogroup"
          aria-label="Crisis scenario selection"
        >
          @for (s of stadiumStore.availableScenarios(); track s.id) {
            ...
            >
              ...
              <div>
                <div class="font-semibold text-sm group-hover:text-amber-400 transition-colors">
                  {{ s.label }}
                </div>
                <div class="text-[10px] text-slate-400 mt-0.5">
                  {{ s.category }} · Level {{ s.severityLevel }}
                </div>
              </div>
            </button>
          } @empty {
            <div class="text-xs text-slate-500 italic py-4 text-center">
              Loading crisis scenarios for this venue...
            </div>
          }
        </div>
      ...
      <div class="space-y-2">
        @if (authService.isGuest()) {
          <p role="note" class="text-[10px] text-slate-500 italic text-center pb-1">
            <span aria-hidden="true">🔒</span> Read-only guest session — triggering is reserved for
        ...
        >
          @if (store.isLoading()) {
            <span
              class="animate-spin rounded-full h-4.5 w-4.5 border-2 border-slate-950 border-t-transparent"
              aria-hidden="true"
            ></span>
          }
        ...
        <div class="grid grid-cols-2 gap-2">
          ...
          >
            ...
            {{ store.isPredicting() ? 'Predicting...' : 'Predict Risk' }}
          </button>
        </div>
      </div>
    </div>
