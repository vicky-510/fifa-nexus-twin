...
@Component({
  ...
  template: `
    <div
      class="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4 font-sans"
    >
      @if (record(); as r) {
        <div
          class="w-full max-w-sm bg-slate-900 border-2 rounded-2xl overflow-hidden shadow-2xl"
          [class]="severityBorder(r.severity)"
          role="region"
          aria-live="polite"
          aria-label="Staff directive card"
        >
          <div class="p-4" [class]="severityBg(r.severity)">
            <div class="text-white font-black text-lg leading-tight">
              <span aria-hidden="true">{{ severityIcon(r.severity) }}</span> {{ r.severity }} —
              {{ r.scenarioLabel }}
            </div>
            <div class="text-white/80 text-xs mt-1">
              {{ r.stadiumName }}{{ r.gate ? ' | ' + r.gate : '' }}
            </div>
          ...
          <div class="p-4 space-y-3">
            <div class="text-center">
              ...
              <div class="text-lg font-bold text-white">
                <span aria-hidden="true">{{ roleInfo.icon }}</span> {{ roleInfo.label }}
              </div>
            </div>

            <div class="bg-slate-950/60 border border-slate-800 rounded-lg p-3">
              <p class="text-sm text-slate-200 leading-relaxed">
                <span aria-hidden="true">▶</span> {{ r.result[roleInfo.key] }}
              </p>
            </div>

            <div class="bg-slate-950/60 border border-slate-800 rounded-lg p-3">
              <span class="text-[10px] uppercase tracking-widest text-slate-500 block mb-1"
                ><span aria-hidden="true">📢</span> Say to crowd</span
              >
            ...
            <div
              class="flex justify-between items-center pt-2 border-t border-slate-800 text-[10px] text-slate-500"
            >
              <span><span aria-hidden="true">⚠️</span> Severity: {{ r.severity }}</span>
              <span
                ><span aria-hidden="true">🕐</span> Next update in: {{ secondsToRefresh() }}s</span
              >
            </div>
          </div>
        </div>
        ...
        <div
          class="text-center text-slate-500 text-sm animate-pulse"
          role="status"
          aria-live="polite"
        >
          Loading directive card...
        </div>
      }
    </div>
  `,
