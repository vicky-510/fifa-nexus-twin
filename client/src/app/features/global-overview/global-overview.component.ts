...
@Component({
  ...
  template: `
    <div class="min-h-screen bg-[#020817] text-slate-100 flex flex-col font-sans page-fade-in">
      ...
      <main class="flex-1 max-w-[1400px] w-full mx-auto p-3 sm:p-6 grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div class="space-y-2">
              @for (stadium of group.stadiums; track stadium.id) {
                ...
                >
                  ...
                  <span class="text-[10px] font-semibold" [style.color]="stadium.color">{{ statusBadge(stadium.status) }}</span>
                </button>
              }
            </div>
          </div>
...
export class GlobalOverviewComponent implements OnInit {
  ...
  countryGroups = computed(() => {
    ...
    const groups: Record<string, { code: string; name: string; flag: string; stadiums: Stadium[] }> = {};

    for (const s of stadiums) {
      if (!groups[s.countryCode]) {
        groups[s.countryCode] = { code: s.countryCode, name: s.country, flag: s.flag, stadiums: [] };
      }
