    ...
    <div class="min-h-screen bg-slate-950 flex flex-col items-center justify-between p-6 text-slate-100 font-sans selection:bg-amber-500 selection:text-slate-950">
      ...
      <div class="w-full max-w-md bg-slate-900/40 border border-slate-800 p-8 rounded-2xl shadow-2xl backdrop-blur-md relative overflow-hidden">
        ...
        <form (ngSubmit)="onSubmit()" #loginForm="ngForm" class="space-y-6">
          ...
          @if (errorMessage()) {
            <div role="alert" aria-live="assertive" class="bg-red-950/40 border border-red-800/80 px-4 py-3 rounded-lg flex items-center space-x-3 text-red-300 text-sm animate-pulse">
              <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5 flex-shrink-0">
          ...
          >
            @if (isLoading()) {
              <svg aria-hidden="true" class="animate-spin h-5 w-5 text-slate-950" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
              ...
              <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2.5" stroke="currentColor" class="w-4 h-4">
