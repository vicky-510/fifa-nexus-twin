...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, AccessibilityToggleComponent],
  template: `
    <div
      class="min-h-screen bg-slate-950 flex flex-col items-center justify-between p-6 text-slate-100 font-sans selection:bg-amber-500 selection:text-slate-950"
    >
      <!-- Top header spacing -->
      <div class="w-full max-w-6xl flex justify-between items-center mb-8">
        <div class="flex items-center space-x-3">
          <div
            class="w-8 h-8 rounded-md bg-amber-500 flex items-center justify-center font-black text-slate-950 tracking-tighter"
          >
            SP
          </div>
          <span class="text-sm uppercase font-bold tracking-widest text-slate-400"
            >StadiumPulse</span
          >
        </div>
      ...
      <div
        class="w-full max-w-md bg-slate-900/40 border border-slate-800 p-8 rounded-2xl shadow-2xl backdrop-blur-md relative overflow-hidden"
      >
        <!-- Cyber decorative top line -->
        <div
          class="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-amber-500 via-yellow-500 to-amber-600"
        ></div>

        <div class="text-center mb-8">
          <div
            class="w-16 h-16 mx-auto mb-4 bg-slate-800/80 border border-slate-700/60 rounded-full flex items-center justify-center text-amber-500 shadow-inner"
          >
            <svg
              aria-hidden="true"
          ...
          <h1 class="text-2xl font-extrabold tracking-tight text-white mb-2">
        ...
        <form (ngSubmit)="onSubmit()" #loginForm="ngForm" class="space-y-6">
          ...
          @if (errorMessage()) {
            <div
              role="alert"
              aria-live="assertive"
              class="bg-red-950/40 border border-red-800/80 px-4 py-3 rounded-lg flex items-center space-x-3 text-red-300 text-sm animate-pulse"
            >
              <svg
                aria-hidden="true"
                xmlns="http://www.w3.org/2000/svg"
          ...
          <button
            ...
            aria-label="Authenticate and enter command center"
        ...
        @if (showColdStartHint()) {
          <p
            role="status"
            aria-live="polite"
            class="text-[11px] text-amber-400/90 text-center mt-3"
          >
            <span aria-hidden="true">⏳</span> First request may take up to a minute while the
            server wakes up from idle — hang tight.
          </p>
        }
      ...
      <div class="text-center text-xs text-slate-500 mt-8 space-y-1">
        ...
        <p class="text-slate-600">
          Access to this system is logged and subject to monitoring. Unauthorized use is prohibited.
        </p>
      </div>
    </div>
  `,
...
export class AccessCodeEntryComponent {
  ...
  private startColdStartTimer(): void {
    this.coldStartTimer = setTimeout(
      () => this.showColdStartHint.set(true),
